...
function getOnlineSorting () {
  chrome.storage.sync.get('OnlineSorting', function (data) {
    if (!chrome.runtime.error) {
      ...
      if (enableOnlineSorting) { onlineSorting(); }
    }
  });
}
...
function onlineSorting () {
  ...
  for (var val in groups) {
    ...
    $(groupHeaders[val]).append(groupHeaders[val]);
    $(groupHeaders[val]).append('(' + groupBox[val].getElementsByTagName('a').length + ')');
    $(groupHeaders[val]).append('<br/>');
    groupsContainer.appendChild(groupBox[val]);
  }
