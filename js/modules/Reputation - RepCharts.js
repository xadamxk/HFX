...
function injectRepLinks () {
  ...
  $('.repvotemid').each(function () {
    var regex = /(https?:\/\/([-\w.]+)+(:\d+)?(\/([_.]*(\?\S+)?)?)?)/ig;
    // Replace plain text links by hyperlinks
    var replacedtext = $(this).find('span:eq(1)').html().replace(regex, "<a href='$1' target='_blank'>$1</a>");
    $(this).find('span:eq(1)').html(replacedtext);
  });
}
...
function injectRepCharts () {
  ...
  if (window.location.href.includes('hackforums.net/reputation.php?uid=')) {
    ...
    // Pie Chart
    posRepTotal = parseInt($('td > span:contains(All Time)').parent().next().find('span').text().replace(/[^\d\.\-]/g, '')); // eslint-disable-line no-useless-escape
    neuRepTotal = parseInt($('td > span:contains(All Time)').parent().next().next().find('span').text().replace(/[^\d\.\-]/g, '')); // eslint-disable-line no-useless-escape
    negRepTotal = parseInt($('td > span:contains(All Time)').parent().next().next().next().find('span').text().replace(/[^\d\.\-]/g, '')); // eslint-disable-line no-useless-escape
  ...
  document.getElementById('repCanvas').onclick = function (evt) {
    ...
    if (firstPoint !== undefined) {
      ...
      switch (labelArray[0]) {
        case 'Positives': { location.href = (window.location.href + '&show=positive'); } // eslint-disable-line no-lone-blocks
          break;
        case 'Neutrals': { location.href = (window.location.href + '&show=neutral'); } // eslint-disable-line no-lone-blocks
          break;
        case 'Negatives': { location.href = (window.location.href + '&show=negative'); } // eslint-disable-line no-lone-blocks
          break;
      }
    }
  };
  ...
  var barOptions_stacked = { // eslint-disable-line camelcase
    title: {
      display: true,
      fontColor: '#cccccc',
      text: 'Timeline'
  ...
  var repChartlastRep = new Chart(repChartlastRepCanvas, { // eslint-disable-line no-unused-vars
    type: 'horizontalBar',
    data: {
