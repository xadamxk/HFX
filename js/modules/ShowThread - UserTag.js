...
function enableUserTag () {
  chrome.storage.sync.get('UserTag', function (data) {
    if (!chrome.runtime.error) {
      ...
      // Run function
      if (enableUserTags) { injectUserTag(); }
    }
  });
}
...
function scanUserTag () {
  ...
  chrome.storage.local.get(['UserTagNameList', 'UserTagThreadTitle'], function (result) {
    ...
    for (var index1 = 0; index1 < tempUsernameList.length; index1++) {
      for (var index2 = 0; index2 < blacklistedUserList.length; index2++) {
    ...
    for (var i = 0; i < tempUsernameList.length; i++) {
      // Only keep first 5 for PM
      if (i < 5) { tempUsernameList[i] = tempUsernameList[i]; } else { tempUsernameList.splice(i, 1); }
    }
    ...
    $.ajax({
      ...
      .done(function (msg) {
        ...
        chrome.storage.local.remove(['UserTagNameList', 'UserTagThreadTitle'], function (result) {
          // Key was removed
          if (debug) { console.log('Previous info was removed.'); }
        });
      })
      .fail(function (msg) {
        if (debug) { console.log('Failed to remove previous queue.'); }
      });
  });
}
