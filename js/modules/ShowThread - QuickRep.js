...
function enableQuickRep () {
  chrome.storage.sync.get("QuickRepEnabled", function (data) {
    if (!chrome.runtime.error) {
      ...
      // Run function
      if (enableQuickReps) { injectQuickRep(); }
    }
  });
}
...
function injectQuickRep () {
  ...
  */
  ...
  repButtonLabel = "Rep"; // Default: "Rep")
  basicQuickRep = false; // (Default: false)
  // Rep comment box width
  repCommentWidth = "60%"; // (Default: "60%")
  // Notification Dismissal Time
  notificationTimeout = 15000; // (Default: 15000)
  // Auto Trigger Rep Queue - otherwise only triggers when out of reps
  queueRep = false; // (Default: false)
  // Global Vars
  uidArray = [];
  errorFound = false;
            ...
            if (!errorFound) {
              ...
              if ($("#repContainerRow").length < 1) {
                ...
                $("body").on("click", "#repPost" + index, function () {
                  ...
                  for (var i = 0; i < recipientUID.length; i++) {
                  ...
                  if ($("#repComment" + index).val().length === 0) {
                    ...
                    $("#repContainerRow").remove();
                  } else if ($("#repComment" + index).val().length < 11 && $("#repComment" + index).val().length > 0) { // Custom comment but too short
                    window.alert("Rep comments must be atleast 10 chars.");
                  } else { // Input over 10 chars
                    // Queue Rep - Custom
                    // TODO: Change statement to this to implement queueRep
                    var queueRepImplemented = false;
                    if (queueRepImplemented) {
...
function submitRepQuest (index) {
  $.ajax({
    ...
    success: function (response) {
      ...
      } else if (errorBlock.includes(repLimit)) { // Rep Limit
        errorFound = true;
        window.alert(permError + repLimit);
      } else if (errorBlock.includes(repSelf)) { // Self rep
        errorFound = true;
        window.alert(permError + repSelfResp);
      } else { // Require Upgrade, Rep Disabled, Other?
        errorFound = true;
        window.alert(permError + errorBlock);
      }
...
function getPrimaryUserGroup () {
  ...
  } else if (desiredString.includes("HF l33t")) { // L33t
    document.cookie = "RepQueueUsergroup=" + "Leet";
  } else if (desiredString.includes("Staff")) { // Staff
...
function removeEntry (queueIndex) {
  ...
  for (var i = 0; i < queuedRep.length - 1; i++) {
